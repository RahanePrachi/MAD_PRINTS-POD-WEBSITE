...
const MyStores = ({ isPublish }) => {
  const [view, setView] = useState("stores"); // "stores" or "settings"

  const [isSetting, setIsSetting] = useState(false)
  const [isPublishComponent, setIsPublishComponent] = useState(isPublish)

  const handleSettingsClick = () => {
    setView("settings");
    setIsSetting(true)
    setIsPublishComponent(false)
    console.log(isPublishComponent, isPublish);
  ...
  return (
    <div style={{ width: "100%" }} className="flex ">
      ...
      {/* Main Content Area */}
      {(isPublishComponent) ? <AddProduct handleSettingClick={handleSettingsClick} /> :
